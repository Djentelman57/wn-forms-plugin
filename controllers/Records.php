...
class Records extends Controller
{
    ...
    {
        $record = Record::with(['mail_outbox_items' => function ($query) {
    ...
    {
        $query->withCount('mail_outbox_items as mail_outbox_total');
        $query->withCount([
            'mail_outbox_items as mail_outbox_sent' => function ($mailQuery) {
                $mailQuery->where('status', 'sent');
            },
            'mail_outbox_items as mail_outbox_failed' => function ($mailQuery) {
                $mailQuery->where('status', 'failed');
            },
            'mail_outbox_items as mail_outbox_processing' => function ($mailQuery) {
                $mailQuery->where('status', 'processing');
            },
            'mail_outbox_items as mail_outbox_queued' => function ($mailQuery) {
                $mailQuery->where('status', 'queued');
            },
            'mail_outbox_items as mail_outbox_pending' => function ($mailQuery) {
                $mailQuery->where('status', 'pending');
            },
        ]);
    }
