...
abstract class MagicForm extends ComponentBase
{
    ...
    public function onFormSubmit()
    {
        ...
        if ($this->property('mail_enabled')) {
            $this->queueMailOutbox('notification', $properties, $post, $record);
        }

        // QUEUE AUTORESPONSE EMAIL
        if ($this->property('mail_resp_enabled')) {
            $this->queueMailOutbox('autoresponse', $properties, $post, $record);
        }
        ...
        // When email notifications are enabled, inform user that delivery can be delayed.
        if ($this->property('mail_enabled') || $this->property('mail_resp_enabled')) {
            $message = Lang::get('martin.forms::lang.components.shared.messages_queued');
        }
    ...
    private function queueMailOutbox(string $mailType, array $properties, array $post, Record $record)
    {
        $outbox = new MailOutbox;
        $outbox->record_id = $record->exists ? $record->id : null;
        $outbox->mail_type = $mailType;
        $outbox->status = MailOutbox::STATUS_PENDING;
